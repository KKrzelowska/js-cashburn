...
const CORRECT_VALUE_TO_SAVE = [
  { date: new Date(), value: 21 },
  { date: new Date(), value: 48 }
];
...
test('When saveValues is executed Then data should be saved in localstorage', () => {
  ...
  expect(localStorage.setItem).toHaveBeenCalledWith(
    memoryService.storageKey,
    JSON.stringify(CORRECT_VALUE_TO_SAVE)
  );
  expect(localStorage.__STORE__[memoryService.storageKey]).toBe(
    JSON.stringify(CORRECT_VALUE_TO_SAVE)
  );
